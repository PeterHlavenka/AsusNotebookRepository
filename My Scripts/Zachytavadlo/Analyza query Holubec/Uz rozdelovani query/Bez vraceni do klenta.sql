exec sp_executesql N'CREATE TABLE ##TempCatchingMessages (Id int, NormCreativeId INT, CreativeId INT, AdvertisedFrom DATETIME, AdvertisedTo DATETIME, CreatedBy TINYINT, ModifiedBy TINYINT, Footage SMALLINT, Modified DATETIME, MediumId SMALLINT, Medium NVARCHAR(255),
                  ...
                  DELETE FROM ##TempCatchingMessages WHERE NormsTapeCode IS NULL  

				  ',N'@from datetime,@to datetime,@fromHelper datetime',@from='2018-08-20 06:59:53',@to='2018-08-20 08:15:23',@fromHelper='2018-08-20 05:59:53'
GO

--INSERT INTO ##TempCatchingMessages (CreativeId ) VALUES (13)

--SELECT * FROM ##TempCatchingMessages

----------------------
--Creativa,  insert creativeToCreativeItem a vymazani duplicit
...
',N'@from datetime,@to datetime',@from='2018-08-20 06:59:53',@to='2018-08-20 08:15:23'
...
JOIN Creative.CreativeToCreativeItem ctci on tcm.CreativeId = ctci.CreativeId
		...
		SELECT DISTINCT CreativeID FROM ##TempCatchingMessages

		SELECT CreativeId, Count(CreativeItemId ) FROM ##TempCatchingMessages
		GROUP BY CreativeId

		SELECT * FROM ##TempCatchingMessages WHERE Id = 100485815

		SELECT * FROM Creative.CreativeToCreativeItem ctci WHERE ctci.CreativeId = 13

		INSERT INTO ##TempCatchingMessages (CreativeId ) VALUES (13)

SELECT * FROM ##TempCatchingMessages AS tcm 
JOIN Creative.Creative cre on tcm.CreativeId = cre.Id
JOIN Creative.CreativeToCreativeItem ctci on tcm.CreativeId = ctci.CreativeId

SELECT * FROM media.MediaMessage WHERE Media.MediaMessage.Id = 100485815